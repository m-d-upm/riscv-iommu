...
module rv_iommu_fq_handler #(
    ...
    input  logic [rv_iommu::VLEN-1:0]              iova_i,             // to report if transaction has an IOVA
    input  logic [rv_iommu::SVX-1:0]               gpaddr_i,           // to report bits [63:2] of the GPA in case of a Guest Page Fault
    ...
    */
    ...
    logic [rv_iommu::VLEN-1:0]             iova;         
    logic [rv_iommu::SVX-1:0]              gpaddr;       
    logic [23:0]                        did;          
    logic                               pv;           
    logic [19:0]                        pid;          
    logic                               is_supervisor;
    ...
    fifo_v3 #(
        .FALL_THROUGH   (1),
        .DEPTH          (4),
        .DATA_WIDTH     (rv_iommu::TTYP_LEN + rv_iommu::CAUSE_LEN + rv_iommu::VLEN + rv_iommu::SVX + 24 + 20 + 4)
    ) i_fifo_fq (
        .clk_i      ( clk_i           ),
        .rst_ni     ( rst_ni          ),
        .flush_i    ( 1'b0            ),
